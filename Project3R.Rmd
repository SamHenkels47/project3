...
mydata$cluster=factor(mydata$cluster,
                           ...
                           labels=c("Largest Export Market","Second Largest Export Market","Major Export Market","Minor Export Market","Very Minor Export Market","Little to No Exports"), 
                           ordered=T)
...
clusterMap= base + geom_sf(data=mapWorldVars,
                           aes(fill=cluster),
                           color="lightgrey") # color of border
clusterMap 
```


```{r}
clusterMap+ scale_fill_brewer(palette ='GnBu',
                               direction = -1)
```
